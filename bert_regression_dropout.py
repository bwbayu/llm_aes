...
class BERTPipeline:
    def __init__(self, config, results, results_epoch):
        ...
        self.model = RegressionModel(config['model_name'], config['dropout']).to(device)
    ...
    def evaluate(self, dataloader, mode="validation"):
        if mode == 'testing':
            self.model = RegressionModel(self.config['model_name'], self.config['dropout']).to(device)
...
df_result = None
if os.path.exists("experiments/results/new_dataset_mix_bert_do1.csv"):
    df_result = pd.read_csv("experiments/results/new_dataset_mix_bert_do1.csv")
    print(df_result['config_id'].iloc[-1])
else:
    print("File 'new_dataset_mix_bert_do1.csv' does not exist.")
...
if os.path.exists("experiments/results/new_dataset_mix_bert_do_epoch1.csv"):
    df_result1 = pd.read_csv("experiments/results/new_dataset_mix_bert_do_epoch1.csv")
    print(max(df_result1['valid_pearson']))
else:
    print("File 'new_dataset_mix_bert_do_epoch1.csv' does not exist.")
...
try:
    ...
    results_path = os.path.join(ROOT_DIR, "experiments/results/new_dataset_mix_bert_do1.csv")
    results_epoch_path = os.path.join(ROOT_DIR, "experiments/results/new_dataset_mix_bert_do_epoch1.csv")
